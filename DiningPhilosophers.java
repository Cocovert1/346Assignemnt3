...
public class DiningPhilosophers {
	...
	public static void main(String[] argv) {
		try {
			...
			// if a command like argument is put
			if (argv.length > 0) {
				try {
					int temp = Integer.parseInt(argv[0]);
					// if negative or zero, do not accept it
					if (temp < 0) {
						System.out.println(argv[0] + " is not a negative integer.");
						System.out.println("\nUsage: java DiningPhilosophers [NUMBER_OF_PHILOSOPHERS]");
						System.exit(0);
					} else if (temp == 0) {
						System.out.println(argv[0] + " is zero.");
						System.out.println("\nUsage: java DiningPhilosophers [NUMBER_OF_PHILOSOPHERS]");
						System.exit(0);
					} else {
						iPhilosophers = temp;
